;(function(window) {
	...
	if (window.BXEventCalendar) // If there is BXEventCalendar obj in window
	{
		window.BXEventCalendar.Core = Calendar; // Then assign Calendar class to BXEventCalendar.Core
	}
	// If there is no BXEventCalendar obj in window, meaning the script is loaded before BXEventCalendar object is created
	// This defer the creation/assignment of the calendar-core library until the main page (index.php) loads the
	// BXEventCalendar object (more in Index.php)
	else 
	{
		BX.addCustomEvent(window, "onBXEventCalendarInit", function() 
		{
			window.BXEventCalendar.Core = Calendar; // Assign Calendar class to BXEventCalendar.Core
		});
	}
})(window);
