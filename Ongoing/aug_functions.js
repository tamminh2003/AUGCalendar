...
async function augAwait(delayTimer, numberOfLoop, timerObject) {
	...
	if (typeof timerObject.testFunc !== "function") {
		...
		return "testFunc of timerObject is not a function";
	};

	if (typeof timerObject.callbackFunc !== "function") {
		console.warn("callbackFunc of timerObject is not a function");
		return "callbackFunc of timerObject is not a function";
	}
		...
		if (augBuildDataCid(listOfElementIDs["Branch Country"]) !== dependentElement.getAttribute("data-name")) {
			...
			if (listOfElementIDs[key]) {
				...
				if (dependentElement.parentElement.parentElement.getAttribute("class").includes("ui-entity-editor-section-edit") && dependentElement !== null) {
					...
					// NOTE: TESTING ON 9/7/2021
					// // Get the previous master element value for validation
					// var previousMasterElement;
					// BX.bind(masterElement, "click", function(){
					//     try {
					//         var masterElement = document.querySelector(augBuildDataCid(listOfElementIDs["Branch Country"]));
					//         var masterDataValue = masterElement.getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0].getAttribute("data-value");
					//         var masterDataValueArray = JSON.parse(masterDataValue);
					//         previousMasterElement = masterDataValueArray;
					//     } catch (error) {
					//         console.log("Error");
					//         return;
					//     }
					...
					// // Reset the dependent element when master element change
					// BX.bind(masterElement, "focusout", (function(dependentElement){
					//     return function(){
					//         try {
					//             var masterElement = document.querySelector(augBuildDataCid(listOfElementIDs["Branch Country"]));
					//             var masterDataValue = masterElement.getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0].getAttribute("data-value");
					...
					//             // Reset all the required fields
					//             var notSelectedOption = {"NAME": "not selected", "VALUE": ""};
					//             if(typeof previousMasterElement !== "undefined"){
					//                 if(masterDataValueArray.NAME !== previousMasterElement.NAME){
					//                     console.log(dependentElement);
					//                     dependentElement.setAttribute("data-value", JSON.stringify(notSelectedOption));
					//                     dependentElement.getElementsByClassName("main-ui-select-name")[0].innerText = notSelectedOption.NAME;
...
/** @function augGenerateArrayValues */
/**
...
* @param {array} staticArray - the static array that is stored in the config file
* @param {integer} idValueFrom - the first id value that wants to store into the array
* @returns {array} newGenerateArray
*/
async function augGenerateArrayValues(customFieldDataCid, staticArray, idValueFrom) {
	try {
		var customFieldElement = document.querySelector(augBuildDataCid(customFieldDataCid));
		var newGenerateArray = {};
			...
			if (typeof customFieldElement.getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0] === "undefined") {
				...
				var augTimerObject = {
					...
					runSelect: function () {
						...
						newGenerateArray = augGenerateArrayValues_(customFieldDataCid, staticArray, idValueFrom);
					}
				}
...
/** @function augGenerateArrayValues_ */
/**
...
* @returns {array} newGenerateArray
*/
function augGenerateArrayValues_(customFieldDataCid, staticArray, idValueFrom) {
	...
	*/
	...
	try {
		...
		// Build new array based on the static array
		// Note: We want to make use of static array to build the filter array first instead of building it when scanning through all the data items one by one
		if (staticArray !== null) {
			for (var i = 0; i < Object.keys(staticArray).length; i++) {
				var key = Object.keys(staticArray)[i];
				for (var j = 0; j < staticArray[key].length; j++) {
					newGenerateArray[staticArray[key][j]] = [];
				}
			}
		}
		...
		for (var i = 0; i < customFieldDataItems.length; i++) {
			...
			if (text.length === 3) {
				...
				if (customFieldDataItems[i].NAME.substring(0, 1) === "*") {
					customFieldDataItems[i].NAME = customFieldDataItems[i].NAME.replace("*", "");
					key = key.replace("*", "");
					checked = true;
				}

				if (validateId === false && parseInt(customFieldDataItems[i].VALUE) >= idValueFrom) {
					validateId = true;
				}

				if (checked === true || validateId === true) {
					if (key !== "--") {
						if (newGenerateArray[key]) {
							newGenerateArray[key].push(customFieldDataItems[i]);
						}
						// If the key is not found
						else {
							console.log("Invalid event name value");
						}
					}
				}
			}
			// 2 layers field
				...
				if (key !== "") {
					...
					// If there are no such key exist then initialise the first array with the value
						...
						newGenerateArray[key] = [customFieldDataItems[i]];
					}
				}
			}
		}
...
* @param {integer} idValueFrom - the first id value that wants to store into the array
*/
async function augBuildThreeLayerAndMockUpFields(customFieldDataCid, threeLayersTextFieldName, mockupTextFieldName, firstSecondOptions, secondThirdOptions, idValueFrom) {
	try {
		...
		if (secondThirdOptions === null) {
			secondThirdOptions = await augGenerateArrayValues(customFieldDataCid, firstSecondOptions, idValueFrom);
		}
...
async function augBuildTwoLayersField(customFieldDataCid, optionsArray, delayTimer, loop) {
	try {
		...
		if (optionsArray === null) {
			optionsArray = await augGenerateArrayValues(customFieldDataCid, null, null);
		}
...
function augSearchSuggestion_(customFieldDataCid, optionsArray) {
	try {
		...
		customFieldInput.parentElement.setAttribute("id", "test-suggestion");

		console.log("Hi");

		// Initialise the search script
		var monkeyList = new List("test-suggestion", {
			valueNames: ['name']
		});
...
/**
* Main function to reload the whole page after saving to reload the script back in (waiting for the button to render)
...
/** @function augReloadPage_ */
/**
* Sub function to reload the whole page after saving to reload the script back in
* @param {integer} numberOfSeconds - the number of seconds for timeout
*/
function augReloadPage_(numberOfSeconds) {
	try {
		...
		BX.bind(saveButton, "click", function () {
			// Note: Alert unable to replace setTimeout as bitrix unable to save the data
			setTimeout(function () {
...
/** @function augFilterAppliedInstitution */
/**
* Main function to build the filter for the Applied Institution, Applied Institution Country, Campus and Applied Institution State (timer loop wait for render)
* @param {object} customFieldDataCidArray - the data-cid attribute of the all the fields (Applied Institution, Applied Institution Country, Campus and Applied Institution State)
* @param {integer} delayTimer - the timer for the await (Example: 1000ms = 1 second)
* @param {integer} loop - the number of loop (Example: 10)
*/
function augFilterAppliedInstitution(customFieldDataCidArray, delayTimer, loop) {
	try {
		// Bind the edit button (only the first element)
		// Assumption: The whole fields array is in the same section
		var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"]));
		if (institutionCountry !== null) {
			var section = institutionCountry.parentElement.parentElement;
			var editButton = section.getElementsByClassName("ui-entity-editor-section-header")[0].getElementsByClassName("ui-entity-editor-header-actions")[0].getElementsByClassName("ui-entity-editor-header-edit-lnk")[0];

			BX.bind(editButton, "click", function () { augFilterAppliedInstitution(customFieldDataCidArray, delayTimer, loop) });

			// Container undefined when clicked on "Edit" text so have to wait for rendering
			if (typeof institutionCountry.getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0] === "undefined") {
				var augTimerObject = {
					className: augBuildDataCid(customFieldDataCidArray["Applied Institution Country"]),

					// Run the main function to set the value
					runSelect: function () {
						augFilterAppliedInstitution_(customFieldDataCidArray)
					},
				};
...
/** @function augFilterAppliedInstitution_ */
/**
* Sub function to build the filter for the Applied Institution, Applied Institution Country, Campus and Applied Institution State (set default values and set filtered value)
* @param {object} customFieldDataCidArray - the data-cid attribute of the all the fields (Applied Institution, Applied Institution Country, Campus and Applied Institution State)
*/
function augFilterAppliedInstitution_(customFieldDataCidArray) {
	try {
		var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
		var institutionCountryDataItems = institutionCountry.getAttribute("data-items");
		var institutionCountryDataItemsString = JSON.parse(institutionCountryDataItems);

		var institution = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
		var institutionDataItems = institution.getAttribute("data-items");
		var institutionDataItemsString = JSON.parse(institutionDataItems);

		// Declare global variable to store the filter array (Only run once)
		if (window.filterInstitutionArray === undefined) {
			window.filterInstitutionArray = {};
		}
		if (window.filterCampusArray === undefined) {
			window.filterCampusArray = {};
		}

		// Check whether the filter institution array is filled (Only run once)
		if (Object.keys(filterInstitutionArray).length === 0) {
			// Create keys for different countries
			for (var i = 0; i < institutionCountryDataItemsString.length; i++) {
				// Ignore the not selected option
				if (institutionCountryDataItemsString[i].NAME !== "not selected") {
					var countryShortName = augcGetCountryShortName[institutionCountryDataItemsString[i].NAME];
					filterInstitutionArray[countryShortName] = [];
				}
			}

			// Insert data into the filter array
			for (var i = 0; i < institutionDataItemsString.length; i++) {
				if (institutionDataItemsString[i].NAME !== "not selected") {
					var country = institutionDataItemsString[i].NAME.substring(0, 2);
					// Validation of the array whether the key exists
					if (filterInstitutionArray[country]) {
						filterInstitutionArray[country].push(institutionDataItemsString[i]);
					}
				}
			}
		}
		...
		var campus = document.querySelector(augBuildDataCid(customFieldDataCidArray["Campus"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
		var campusDataItems = campus.getAttribute("data-items");
		var campusDataItemsString = JSON.parse(campusDataItems);

		// Check whether the filter campus array is filled (Only run once)
		if (Object.keys(filterCampusArray).length === 0) {
			// Create keys for different countries
			for (var i = 0; i < institutionCountryDataItemsString.length; i++) {
				// Ignore the not selected option
				if (institutionCountryDataItemsString[i].NAME !== "not selected") {
					var countryShortName = augcGetCountryShortName[institutionCountryDataItemsString[i].NAME];
					filterCampusArray[countryShortName] = [];
				}
			}

			// Insert data into the filter array
			for (var i = 0; i < campusDataItemsString.length; i++) {
				if (campusDataItemsString[i].NAME !== "not selected") {
					var country = campusDataItemsString[i].NAME.substring(0, 2);
					// Validation of the array whether the key exists
					if (filterCampusArray[country]) {
						filterCampusArray[country].push(campusDataItemsString[i]);
					}
				}
			}
		}

		// Hide Applied Institution State
		var state = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution State"]));
		BX.addClass(state, "aug_hide");


		// Load default value into data-items (Applied Institution and Campus) using Bind (master and dependent)
		var institutionCountryDataValue = institutionCountry.getAttribute("data-value");
		var institutionCountryDataValueString = JSON.parse(institutionCountryDataValue);
		var country = institutionCountryDataValueString.NAME;


		// Remove the "not selected" from Applied Institution Country
		// Assumption: "not selected" is the first value
		institutionCountryDataItemsString = institutionCountryDataItemsString.splice(1, institutionCountryDataItemsString.length - 1);
		institutionCountry.setAttribute("data-items", JSON.stringify(institutionCountryDataItemsString));


		// Bind the Applied Institution based on the Applied Institution Country (default)
		// Master element = institution country
		// Dependent element = institution
		var previousInstitutionSelected;
		BX.bind(institution, "click", function () {
			try {
				var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionCountryDataValue = institutionCountry.getAttribute("data-value");
				var institutionCountryDataValueString = JSON.parse(institutionCountryDataValue);
				...
				var institution = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionDataValue = institution.getAttribute("data-value");
				var institutionDataValueString = JSON.parse(institutionDataValue);

				console.log(institutionCountryDataValueString);

				// Show specific institution based on country where if "not selected" will show everything (default)
				if (institutionCountryDataValueString.NAME !== "not selected") {
					var country = augcGetCountryShortName[institutionCountryDataValueString.NAME];
					console.log(country);
					console.log(filterInstitutionArray);
					console.log(filterInstitutionArray[country]);

					// Set default institution fields
					if (Object.keys(filterInstitutionArray).length !== 0) {
						console.log("Enter");
						console.log(institution);
						institution.setAttribute("data-items", JSON.stringify(filterInstitutionArray[country]));
					}
				}

				previousInstitutionSelected = institutionDataValueString;
				console.log(previousInstitutionSelected);
		...
		// Bind the Campus field based on Applied Institution Country (default)
		// Master element = institution country
		// Dependent element = campus
		BX.bind(campus, "click", function () {
			try {
				var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionCountryDataValue = institutionCountry.getAttribute("data-value");
				var institutionCountryDataValueString = JSON.parse(institutionCountryDataValue);
				...
				// No institution is selected then no campus
				var notSelectedOption = [{ "NAME": "not selected", "VALUE": "" }];
				if (institutionDataValueString.NAME === "not selected") {
					campus.setAttribute("data-items", JSON.stringify(notSelectedOption));
				}
				else {
					// Show specific campus based on country where if "not selected" will show everything (default)
					if (institutionCountryDataValueString.NAME !== "not selected") {
						var country = augcGetCountryShortName[institutionCountryDataValueString.NAME];

						// Set default campus fields
						if (Object.keys(filterCampusArray).length !== 0) {
							campus.setAttribute("data-items", JSON.stringify(filterCampusArray[country]));
						}

						// Set defined campus fields based on the Institutions
						if (institutionDataValueString.NAME.includes("[")) {
							// Search for regular expression inside "[]"
							var regText = institutionDataValueString.NAME.match(/\[(.*?)\]/);
							if (regText) {
								var squareBracketCampus = regText[1].split(",");
								var refineCampusArray = [];

								for (var i = 0; i < squareBracketCampus.length; i++) {
									// Follow back the original format
									squareBracketCampus[i] = country + "--" + squareBracketCampus[i];
									for (var j = 0; j < filterCampusArray[country].length; j++) {
										if (squareBracketCampus[i] === filterCampusArray[country][j].NAME) {
											refineCampusArray.push(filterCampusArray[country][j]);
											break;
										}
									}
								}
								campus.setAttribute("data-items", JSON.stringify(refineCampusArray));
							}
						}
					}
				}
		...
		// Bind the Applied Institution State based on Campus field (focusout)
		BX.bind(campus, "focusout", function () {
			try {
				var campus = document.querySelector(augBuildDataCid(customFieldDataCidArray["Campus"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var campusDataValue = campus.getAttribute("data-value");
				var campusDataValueString = JSON.parse(campusDataValue);

				// If there are corresponding states in the pre-defined array then set the institution state
				if (augcAppliedInstitutionState[campusDataValueString.NAME]) {
					var institutionState = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution State"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
					var institutionStateDataItems = institutionState.getAttribute("data-items");
					var institutionStateDataItemsString = JSON.parse(institutionStateDataItems);

					for (var i = 0; i < institutionStateDataItemsString.length; i++) {
						if (parseInt(institutionStateDataItemsString[i].VALUE) === augcAppliedInstitutionState[campusDataValueString.NAME]) {
							institutionState.setAttribute("data-value", JSON.stringify(institutionStateDataItemsString[i]));
						}
					}

					// IMPORTANT: It triggers the built in save function to save the data to the bitrix itself (a mockup of validation html)
					institutionState.click();
				}
			} catch (error) {
				console.log("Error");
				return;
		...
		// Get the previous country selected value for validation
		var previousCountrySelected;
		BX.bind(institutionCountry, "click", function () {
			try {
				var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionCountryDataValue = institutionCountry.getAttribute("data-value");
				var institutionCountryDataValueString = JSON.parse(institutionCountryDataValue);
				previousCountrySelected = institutionCountryDataValueString;
			} catch (error) {
				console.log("Error");
				return;
			}
		...
		// Reset Applied Institution, Campus and Applied Institution State when Applied Institution Country change
		BX.bind(institutionCountry, "focusout", function () {
			try {
				console.log(previousCountrySelected);
				var institutionCountry = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution Country"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionCountryDataValue = institutionCountry.getAttribute("data-value");
				var institutionCountryDataValueString = JSON.parse(institutionCountryDataValue);
				...
				// Reset all the required fields
				var notSelectedOption = [{ "NAME": "not selected", "VALUE": "" }];
				if (typeof previousCountrySelected !== "undefined") {
					if (institutionCountryDataValueString.NAME !== previousCountrySelected.NAME) {
						institution.setAttribute("data-value", JSON.stringify(notSelectedOption));
						institution.getElementsByClassName("main-ui-select-name")[0].innerText = notSelectedOption.NAME;

						campus.setAttribute("data-value", JSON.stringify(notSelectedOption));
						campus.getElementsByClassName("main-ui-select-name")[0].innerText = notSelectedOption.NAME;

						institutionState.setAttribute("data-value", JSON.stringify(notSelectedOption));
						institutionState.getElementsByClassName("main-ui-select-name")[0].innerText = notSelectedOption.NAME;
		...
		// Reset Campus and Applied Institution State when Applied Institution change
		BX.bind(institution, "focusout", function () {
			try {
				console.log(previousInstitutionSelected);
				var institution = document.querySelector(augBuildDataCid(customFieldDataCidArray["Applied Institution"])).getElementsByClassName("ui-entity-editor-content-block")[0].getElementsByTagName("div")[0];
				var institutionDataValue = institution.getAttribute("data-value");
				...
				// Reset all the required fields
				var notSelectedOption = [{ "NAME": "not selected", "VALUE": "" }];
				if (typeof previousInstitutionSelected !== "undefined") {
					if (institutionDataValueString.NAME !== previousInstitutionSelected.NAME) {
						campus.setAttribute("data-value", JSON.stringify(notSelectedOption));
						campus.getElementsByClassName("main-ui-select-name")[0].innerText = notSelectedOption.NAME;
